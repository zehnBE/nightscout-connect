...
function nightscoutRestAPI (config, axios) {
  ...
  // Update bookmark from treatments if treatment timestamp is newer than current bookmark
  ...
  function record_batch (batch) {
    ...
    return Promise.all([
        ...
        // Pass original treatments array (not API response) to bookmark_from_treatments
  ...
  record_batch.gap_for = function ( ) {
    ...
    // Try entries first, fall back to treatments if empty
    return http.get('/api/v1/entries.json', { params: { count: 1 }, headers }).then((resp) => {
      ...
      console.log("FAILED TO DETERMINE GAP FROM ENTRIES", err.request, err.response && err.response.status, err.response && err.response.data);
    }).then(( ) => {
      // If no entries bookmark, try treatments
      if (!bookmark.entries) {
