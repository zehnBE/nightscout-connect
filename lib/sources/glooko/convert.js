...
function generate_nightscout_treatments(batch, timestampDelta) {
  ...
  // pumpTimestamp comes as UTC ISO string from API - use directly, no offset
  if (pumpBoluses) {
    pumpBoluses.forEach(function(element) {
      var treatment       = {};
      treatment.eventType = 'Meal Bolus';
      treatment.eventTime = element.pumpTimestamp;
  ...
  if (scheduledBasals) {
    scheduledBasals.forEach(function(element) {
      ...
      treatment.created_at = element.pumpTimestamp;
