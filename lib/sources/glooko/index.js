  ...
  var impl = {
    authFromCredentials () {
      ...
      return http.post(Defaults.login, payload).then((response) => {
        ...
        if (err.response) {
          console.log('GLOOKO AUTH FAILED: HTTP', err.response.status, JSON.stringify(err.response.data || ''));
        } else {
    ...
    dataFromSesssion (session, last_known) {
      ...
      return Promise.all([
        ...
        fetcherV3(v3Url),                      // v3 - CGM + basalBarAutomated (immer 2 Tage)
    ...
    transformData (batch) {
      ...
      var entries    = v3SeriesToEntries(batch.cgmSeriesV3);
