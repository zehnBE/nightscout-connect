...
function glookoSource (opts, axios) {
  ...
  // Stored outside dataFromSesssion so it persists across loops
  var lastUpdatedAt = {
  ...
  var impl = {
    ...
    dataFromSesssion (session, last_known) {
      ...
      /**
       * Build v2 URL.
       * lastUpdatedAtFilter: use stored updatedAt for diff, fallback to startISO.
       */
      ...
      /**
       * Extract max updatedAt from a list of records.
       */
      ...
      function fetcherV2Diff(endpoint, itemsKey, storedKey) {
        ...
        return http.get(fullUrl, { headers }).then(function(resp) {
          ...
          // If diff returned 0 results and we were in diff mode,
          // fall back to full fetch as safety net
          if (isDiff && items.length === 0) {
            console.log('GLOOKO V2 DIFF empty - falling back to full fetch');
            var fullHeaders = makeHeaders();
            var fullFullUrl = buildV2Url(endpoint, startISO);
            return http.get(fullFullUrl, { fullHeaders }).then(function(resp2) {
              var data2  = resp2.data;
              var items2 = data2[itemsKey] || [];
              console.log('GLOOKO V2 FULL FALLBACK - got', items2.length, itemsKey);
              // Update stored updatedAt from full fetch
              var newMax = maxUpdatedAt(items2);
              if (newMax) {
          ...
          // Update stored updatedAt from new records
          var newMax = maxUpdatedAt(items);
          if (newMax) {
            // Only advance if newer
