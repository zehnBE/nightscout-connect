...
function glookoSource (opts, axios) {
  ...
  var impl = {
    ...
    dataFromSesssion (session, last_known) {
      ...
      return Promise.all([
        fetcherV2(Defaults.LatestPumpBasals),                          // v2 - scheduledBasals
        fetcherV2(Defaults.LatestPumpBolus),                           // v2 - normalBoluses
        cgmEnabled ? fetcherV3(v3Url) : Promise.resolve(null),         // v3 - CGM (skipped if disabled)
    ...
    transformData (batch) {
      ...
      // CONNECT_GLOOKO_CGM=0/off: skip CGM entries, return treatments only
      var cgmEnvVal  = process.env.CONNECT_GLOOKO_CGM;
      var cgmEnabled = !(cgmEnvVal === '0' || (cgmEnvVal && cgmEnvVal.toLowerCase() === 'off'));
