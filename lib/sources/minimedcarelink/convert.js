...
//  Timestamp-Handling
//
// sg.datetime ist Lokalzeit der Pumpe (z.B. CEST), aber mit Z oder -00:00 markiert.
// lastConduitDateTime hat den echten Timezone-Offset (+02:00 fuer CEST).
// Korrekte UTC-Zeit = isoToMs(sg.datetime) - extractTzOffsetMs(raw)
// Dadurch stabile, zyklus-unabhaengige Timestamps fuer Nightscout-Dedup.

function isoToMs(ts)
{
  if (!ts) { return null; }
  var s = ts.replace(' ', 'T');
  if (s.indexOf('Z') === -1 && s.indexOf('+') === -1 && !s.match(/[+-]\d{2}:\d{2}$/)) { s += 'Z'; }
  var ms = Date.parse(s);
  return isNaN(ms) ? null : ms;
}

function extractTzOffsetMs(raw)
{
  // lastConduitDateTime: '2026-04-02T11:15:13.240+02:00'
  var lc = raw.lastConduitDateTime || '';
  var m  = lc.match(/([+-])(\d{2}):(\d{2})$/);
  if (!m) { return 0; }
  var sign = (m[1] === '+') ? 1 : -1;
...
{
  // sg.datetime ist stabil (kommt von Pumpe), aber als Lokalzeit mit falscher Z-Markierung.
  // Offset aus lastConduitDateTime abziehen fuer korrektes UTC.
  var ms = isoToMs(sg.datetime || sg.timestamp);
  if (ms) { return ms - extractTzOffsetMs(raw); }
  return null;
}
...
function extractTreatments(raw)
{
  ...
  {
    if (!m.type || (!m.timestamp && !m.dateTime)) { return; }
    var ts = (raw.currentServerTime && m.relativeOffset !== undefined)
      ? Math.round((raw.currentServerTime + m.relativeOffset * 1000) / 60000) * 60000
      : isoToMs(m.dateTime || m.timestamp);
    var dv = m.data && m.data.dataValues ? m.data.dataValues : m;
