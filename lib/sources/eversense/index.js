      ...
      return axios.post(tokenUrl, params.toString(), {
        ...
        if (status === 400 || status === 401) {
          if (data.error_code === 5005 || (data.error_description && data.error_description.indexOf('locked') !== -1)) {
            console.error('EVERSENSE_AUTH_ERROR: ACCOUNT_LOCKED  ' + body);
            throw new Error('EVERSENSE_AUTH_ERROR: ACCOUNT_LOCKED  Konto gesperrt, bitte im Eversense-Portal entsperren');
          }
    ...
    dataFromSesssion: function(session, last_known)
    {
      ...
      return tokenPromise.then(function(td)
      {
        ...
        }).then(function(dashR)
        {
          ...
          // +1ms: letzten bekannten Eintrag ausschliessen
          var from = new Date(Math.max(lastKnownMs + 1, now.getTime() - 24 * 60 * 60 * 1000));
