...
//  var frameName = 'frameGLOOKO';
  ...
  const loopConfig = {
    ...
    actions: {
      ...
      // xstate after-delays use unref'd setTimeout which allows Node.js to exit.
      // This creates a ref'd timer to keep the process alive until the next loop.
      keepAlive: (context, event) => {
        var t = setTimeout(function() { }, config.expected_data_interval_ms + 1000);
        if (t && t.unref) {
          // explicitly ref it to keep Node alive
          t.ref();
        }
      },
    },
